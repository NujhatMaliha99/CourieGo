...
-- Required default sender because the current frontend has no sender-management page.
IF EXISTS (SELECT 1 FROM dbo.users WHERE email = 'sender@example.com')
   AND NOT EXISTS (SELECT 1 FROM dbo.users WHERE email = 'sender@couriego.local')
BEGIN
    UPDATE dbo.users
    SET full_name = 'Default Sender',
        email = 'sender@couriego.local',
        phone = NULL
    WHERE email = 'sender@example.com';
END;

IF NOT EXISTS (SELECT 1 FROM dbo.users WHERE email = 'sender@couriego.local')
BEGIN
    INSERT INTO dbo.users (role_id, full_name, email, phone, address)
    SELECT role_id, 'Default Sender', 'sender@couriego.local', NULL, 'Dhaka'
    FROM dbo.roles
